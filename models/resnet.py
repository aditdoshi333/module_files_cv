    ...
    def __init__(self, in_planes, planes,norm_type, stride=1):
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                ...
                nn.BatchNorm2d(self.expansion*planes) if norm_type == 'BatchNorm' else nn.GroupNorm(1,self.expansion*planes)
            )
...
def ResNet18(norm_type='BatchNorm'):
    if norm_type == 'BatchNorm':
    	return ResNet(BasicBlock, [2, 2, 2, 2],'BatchNorm')
    else:
        return ResNet(BasicBlock, [2, 2, 2, 2],'LayerNorm')


def ResNet34(norm_type='BatchNorm'):
    if norm_type == 'BatchNorm':
    	return ResNet(BasicBlock, [3, 4, 6, 3],'BatchNorm')
    else:
        return ResNet(BasicBlock, [3, 4, 6, 3],'LayerNorm')
