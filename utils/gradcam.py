...
def imshow(image,predicted_label, target_label, target_classes ):
    image = image.numpy()
    fig = plt.figure(figsize=(10,10))
    plt.imshow(np.transpose(image, (1, 2, 0)),interpolation='none')
    plt.title(f'Predicted = {target_classes[int(predicted_label)]} \n Target = {target_classes[int(target_label)]}')

def grad_cam_plot(model, target_classes, list_of_misclassified_images):
  gradcam = GradCAM.from_config(model_type='resnet', arch=model, layer_name='layer4')
  for misclassified_image in range(20):
    grad_cam_image_result = [denormalize(list_of_misclassified_images[misclassified_image][0].unsqueeze(0),(0.4914, 0.4822, 0.4465) , (0.2023, 0.1994, 0.2010)).squeeze().cpu()]
    ...
    imshow(grid_image, predicted_label, target_label, target_classes)
